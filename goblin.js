(function () {
    ...
    // Загрузка через Lampa.Reguest — корректно обрабатывает CORS и таймауты
    function fetchFeed(onOk, onErr) {
        ...
        net.native(FEED_URL, function (data) {
            ...
            // fallback на HTTP, если HTTPS не прошёл
            net.native('http://oper.ru/video.xml', function (data2) {
                try {
                    var videos = parseXML(data2);
                    setCache(videos);
                    onOk(videos);
                } catch (e) { onErr('Парсинг (HTTP): ' + e.message); }
    ...
    function parseXML(xmlText) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            // Принудительно HTTPS для URL и постеров
    ...
    function GoblinComponent(activity) {
        ...
        self.showLoading = function () {
            self.scroll.clear();
            self.scroll.append('<div class="goblin-loading"><div class="broadcast__scan"></div><div>Загрузка ленты…</div></div>');
        };
    ...
    function start() {
        ...
        // Добавляем в меню (как у тебя сработало)
